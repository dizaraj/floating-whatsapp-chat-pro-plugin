...
// Define the remote verification server URL
define('WCW_PRO_LICENSE_SERVER_URL', 'https://whatsapp-pro-chat.web.app/');
 ...
 * @return bool
 * @since 2.5.0
 * @updated 2.9.1 - Now relies on 'api_key_status' and 'api_key_expires' from options,
 * which are set by remote verification or internal logic.
 */
function wcw_pro_is_pro() {
    ...
    // For trial keys, check status and expiration
...
function wcw_pro_get_visitor_public_ip_and_location() {
    ...
    // Use wp_safe_remote_get for external HTTP requests
...
function wcw_pro_add_admin_menu() {
    ...
    add_submenu_page(
        ...
        'manage_options',
        'wcw-pro', // This makes it the default page
        'wcw_pro_settings_page_html'
    );
...
/**
 * Custom styles for the admin menu.
 * @since 2.9.0
 */
function wcw_pro_admin_menu_styles() {
    ?>
    <style>
...
add_action( 'admin_head', 'wcw_pro_admin_menu_styles' );
 ...
 * Includes remote license verification.
 * @param array $input The settings input from the form.
 * @return array The sanitized and updated settings.
 * @since 2.6.8
 * @updated 2.9.2 - Handle HTML-wrapped JSON responses from the license server.
 */
function wcw_pro_sanitize_settings( $input ) {
    ...
    if (isset($input['api_key'])) {
        ...
        // This prevents re-validation if the user didn't explicitly change the key.
        ...
        // Only run validation if the key is not empty and has actually been changed or is new.
        if ( !empty($api_key) && ($api_key !== ($options['api_key'] ?? '') || strpos($submitted_key, '✱') === false) ) {
            ...
            // Remote verification for standard PRO keys
                ...
                $response = wp_safe_remote_get($verify_url, [
                    ...
                    'headers' => [ 'Accept' => 'application/json' ], // Be explicit about what we want
                ]);
                ...
                if (is_wp_error($response)) {
                    ...
                    if ($response_code !== 200) {
                        ...
                        // FIX: If the primary JSON decode fails, the server might have wrapped the JSON in an HTML document.
                        // Attempt to strip HTML tags and decode the remaining text content.
                        ...
                        // After potential fallback, check if we have valid data and a status.
                        if (json_last_error() === JSON_ERROR_NONE && isset($data['status'])) {
                            ...
                            } else { // 'invalid' or other statuses from server
                            ...
                            // If all decoding attempts fail or status is missing.
                            $new_input['api_key_status'] = 'invalid';
                            $new_input['api_key_expires'] = 0;
    ...
    if ( isset( $input['agents'] ) && is_array( $input['agents'] ) ) {
        ...
        // Re-check pro status based on potentially new input's API key status
                ...
                <div class="wcw-pro-settings-card">
                    ...
                    <div class="wcw-pro-card-body">
                        ...
                        <ul class="wcw-pro-contact-list">
                            ...
                            <li><strong>Messenger:</strong> <a href="https://m.me/dizaraj" target="_blank">m.me/dizaraj</a></li>
 ...
 * @since 2.6.8
 * @updated 2.9.3 - Added "Get Pro License" button.
 */
...
function wcw_pro_handle_trial_request() {
    ...
    $to = 'dizaraj@gmail.com'; // Developer's email
    ...
    // Attempt to send email, log error if it fails
...
function wcw_pro_render_frontend_widget() {
    ...
    // If no agents are set, or no valid license and more than 1 agent, return early.
    if ( empty( $agents ) ) {
        return;
    }
    
    // Enforce single agent if not pro
    if (!$is_pro) {
        ...
        <div class="wcw-pro-popup">
            ...
            <div class="wcw-pro-body">
                <?php foreach ( $agents as $agent ) : ?>
                    ...
                    // Ensure image is retrieved at appropriate size, or fallback to placeholder
...
function wcw_pro_enqueue_assets() {
    wp_enqueue_style( 'wcw-pro-style', plugin_dir_url( __FILE__ ) . 'assets/css/style.css', array(), '2.9.3' );
    wp_enqueue_script( 'wcw-pro-script', plugin_dir_url( __FILE__ ) . 'assets/js/main.js', array( 'jquery' ), '2.9.3', true );
}
...
function wcw_pro_enqueue_admin_scripts( $hook ) {
    ...
    wp_enqueue_style( 'wcw-pro-admin-style', plugin_dir_url( __FILE__ ) . 'assets/css/admin.css', array(), '2.9.3' );
    wp_enqueue_script( 'wcw-pro-admin-script', plugin_dir_url( __FILE__ ) . 'assets/js/admin.js', array( 'jquery', 'wp-util', 'wp-color-picker', 'jquery-ui-accordion' ), '2.9.3', true );
