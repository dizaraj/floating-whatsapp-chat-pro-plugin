...
function wcw_pro_settings_init() {
    ...
    // WooCommerce Section
    add_settings_section('wcw_pro_woocommerce_section', null, null, 'wcw_pro_settings_woocommerce');
    add_settings_field('wcw_pro_wc_enable_field', esc_html__('Product Page Button', 'wcw-pro'), 'wcw_pro_render_field', 'wcw_pro_settings_woocommerce', 'wcw_pro_woocommerce_section', ['type' => 'checkbox', 'id' => 'wc_button_enable', 'description' => 'Enable the "Chat on WhatsApp" button on single product pages.']);
...
function wcw_pro_sanitize_settings( $input ) {
    ...
    $new_input['wc_button_enable'] = isset( $input['wc_button_enable'] ) ? 1 : 0;
        ...
        <div class="wcw-pro-settings-content">
            <div class="wcw-pro-settings-main">
                ...
                <form action="options.php" method="post">
                    ...
                    <div id="woocommerce" class="wcw-pro-tab-content">
                         <?php do_settings_sections('wcw_pro_settings_woocommerce'); ?>
                    </div>
...
function wcw_pro_add_wc_button() {
    $options = get_option('wcw_pro_settings', []);
    if ( ! class_exists( 'WooCommerce' ) || empty($options['wc_button_enable']) ) return;
...
function wcw_pro_enqueue_assets() {
    wp_enqueue_style( 'wcw-pro-style', plugin_dir_url( __FILE__ ) . 'assets/css/style.css', array(), '2.1.1' );
    wp_enqueue_script( 'wcw-pro-script', plugin_dir_url( __FILE__ ) . 'assets/js/main.js', array( 'jquery' ), '2.1.1', true );
}
...
function wcw_pro_enqueue_admin_scripts( $hook ) {
    ...
    wp_enqueue_style( 'wcw-pro-admin-style', plugin_dir_url( __FILE__ ) . 'assets/css/admin.css', array(), '2.1.1' );
    wp_enqueue_script( 'wcw-pro-admin-script', plugin_dir_url( __FILE__ ) . 'assets/js/admin.js', array( 'jquery', 'wp-util', 'wp-color-picker', 'jquery-ui-accordion' ), '2.1.1', true );
}
    ...
    if ( false === get_option( 'wcw_pro_settings' ) ) {
        ...
        if ( ! empty( $old_widget_settings ) && is_array( $old_widget_settings ) ) {
            ...
            $defaults = [
                ...
                'icon_color' => '#FFFFFF',
                'wc_button_enable' => 1,
                'agents' => [[ 'name' => 'Support Team', 'title' => 'Customer Support', 'department' => 'Sales', 'phone' => '1234567890', 'message' => 'Hello! I have a question.' ]]
            ];
        }
