...
Description: Adds a customizable floating WhatsApp chat widget to your website. Manage multiple agents and customize the widget to match your brand.
Version: 2.8.0
Author: Dizaraj Dey
...
/**
 * Get visitor's public IP. This is the person viewing the page.
 * @return string
 * @since 2.7.0
 */
...
function wcw_pro_get_visitor_public_ip_and_location() {
    ...
    // Return a default structure if IP is not found.
    if (empty($ip)) {
    ...
    // Fallback if the API fails, return the detected IP.
...
function wcw_pro_get_essential_info() {
    global $wp_version;
    // When called on the admin dashboard, the "visitor" is the admin.
...
/**
 * Add the admin menu page and sub-menu pages.
 * @since 2.8.0
 */
function wcw_pro_add_admin_menu() {
    add_menu_page(
        esc_html__( 'WhatsApp Chat Pro', 'wcw-pro' ),
        esc_html__( 'WhatsApp Chat', 'wcw-pro' ),
        'manage_options',
        'wcw-pro-settings', // Parent slug
    ...
    add_submenu_page(
        'wcw-pro-settings', // Parent slug
        esc_html__( 'Activate License', 'wcw-pro' ),
        esc_html__( 'Activate', 'wcw-pro' ),
        'manage_options',
        'wcw-pro-activate',
        'wcw_pro_activate_page_html'
    );
...
function wcw_pro_settings_init() {
    ...
    // General Section
    add_settings_section('wcw_pro_general_section', null, 'wcw_pro_general_settings_fields_callback', 'wcw_pro_settings_general');
    
    // Display Section
    add_settings_section('wcw_pro_display_section', null, 'wcw_pro_display_settings_fields_callback', 'wcw_pro_settings_display');
    ...
    // License Section (now on its own page, but fields are registered here)
    ...
    add_settings_field('wcw_pro_api_key_field', esc_html__('License Key', 'wcw-pro'), 'wcw_pro_api_key_field_callback', 'wcw_pro_settings_license', 'wcw_pro_license_section');
...
/**
 * Render the General section fields in a card layout.
 * @since 2.6.6
 */
function wcw_pro_general_settings_fields_callback() {
    ?>
        ...
        <div class="wcw-pro-settings-card">
            ...
            <div class="wcw-pro-card-body">
                <div class="wcw-pro-form-group">
                    ...
                    <?php wcw_pro_render_field(['type' => 'text', 'id' => 'popup_title', 'default' => 'Start a Conversation', 'placeholder' => 'Start a Conversation']); ?>
                    <p class='description'>The title displayed at the top of the chat popup.</p>
                </div>
                <div class="wcw-pro-form-group">
                    <label><?php esc_html_e('Button Position', 'wcw-pro'); ?></label>
                    <?php wcw_pro_button_position_field_callback(); ?>
...
/**
 * Render the Display & Styling section fields.
 * @since 2.6.4
 */
function wcw_pro_display_settings_fields_callback() {
    ?>
    <div class="wcw-pro-settings-grid">
        <div class="wcw-pro-settings-card">
            <div class="wcw-pro-card-header">
            ...
            <div class="wcw-pro-card-body">
                <div class="wcw-pro-form-group">
                    ...
                    <?php wcw_pro_render_field(['type' => 'color', 'id' => 'bubble_color', 'default' => '#25D366']); ?>
                </div>
                <div class="wcw-pro-form-group">
                    <label for="icon_color"><?php esc_html_e('Icon & Text Color', 'wcw-pro'); ?></label>
                    <?php wcw_pro_render_field(['type' => 'color', 'id' => 'icon_color', 'default' => '#FFFFFF']); ?>
                </div>
            </div>
        </div>
        <div class="wcw-pro-settings-card">
            <div class="wcw-pro-card-header">
                 <h3><span class="dashicons dashicons-toggle-on"></span> Button Configuration</h3>
            </div>
            <div class="wcw-pro-card-body">
                 <div class="wcw-pro-form-group">
                    <label><?php esc_html_e('Button Style', 'wcw-pro'); ?></label>
                    <?php wcw_pro_button_style_field_callback(); ?>
                </div>
                <div class="wcw-pro-form-group" id="button_text_field_group" style="display:none;">
                    ...
                    <?php wcw_pro_render_field(['type' => 'text', 'id' => 'button_text', 'default' => 'Need Help!', 'placeholder' => 'Need Help!']); ?>
                </div>
            </div>
        </div>
    </div>
...
function wcw_pro_sanitize_settings( $input ) {
    ...
    // Persist existing license status and expiration unless changed by new key submission.
    ...
    if (isset($input['api_key'])) {
        ...
        // If the submitted key contains the mask character, use the already saved key.
        ...
        // Only run validation if the key is not empty and has actually been changed by the user.
        if ( !empty($api_key) && strpos($submitted_key, '✱') === false ) {
            if ( $api_key === 'PRO-UNLOCK-2025' ) {
                ...
                $new_input['api_key_status'] = 'invalid';
                $new_input['api_key_expires'] = 0;
                add_settings_error('wcw_pro_settings', 'api_key_invalid', 'The License key you entered is not valid.', 'error');
            }
        }
    } else {
        // Fallback to ensure the key is not lost if the field is not in the submission.
 ...
 * @since 2.6.6
 */
function wcw_pro_button_position_field_callback() {
    $options = get_option('wcw_pro_settings');
...
/**
 * Render the Activate page HTML structure.
 * @since 2.8.0
 */
function wcw_pro_activate_page_html() {
    ...
    <div class="wrap wcw-pro-settings-wrap">
        ...
        <form action="options.php" method="post">
            ...
            <div class="wcw-pro-settings-main" style="flex:unset; max-width: 800px;">
                 <div class="wcw-pro-settings-card">
                    <div class="wcw-pro-card-header">
                        <h3><span class="dashicons dashicons-vpn-key"></span> License & Activation</h3>
                    </div>
                    <div class="wcw-pro-card-body">
...
/**
 * Render the About page HTML structure.
 * @since 2.8.0
 */
function wcw_pro_about_page_html() {
    if ( ! current_user_can( 'manage_options' ) ) return;
    ?>
    <div class="wrap wcw-pro-settings-wrap">
        <h1><?php echo esc_html( get_admin_page_title() ); ?></h1>
            ...
            <div class="wcw-pro-settings-main">
                <div class="wcw-pro-settings-card">
                    ...
                    <div class="wcw-pro-card-body">
                        ...
                        <ol>
                            ...
                            <li>Under the <strong>Display & Styling</strong> tab, customize the colors and button style to match your brand.</li>
                            <li>Under the <strong>Agents</strong> tab, add at least one support agent. You must provide a name and a valid WhatsApp number (including country code).</li>
                            <li>Click <strong>Save Settings</strong>.</li>
                        </ol>
                        <h4>Activating Pro Features:</h4>
                        <ol>
                            <li>Navigate to <strong>WhatsApp Chat > Activate</strong>.</li>
                            <li>You can request a 7-day trial key by entering your email.</li>
                            <li>Once you have a license key (either trial or full), enter it in the 'License Key' field and click <strong>Save & Activate</strong>.</li>
                            <li>With a Pro license, you can add multiple agents, upload custom avatars, and assign departments.</li>
                        </ol>
                    </div>
                </div>

                <div class="wcw-pro-settings-card" style="margin-top: 20px;">
                    <div class="wcw-pro-card-header">
                        <h3><span class="dashicons dashicons-businessman"></span> About the Developer</h3>
                    </div>
                    <div class="wcw-pro-card-body">
                        <p>This plugin is developed and maintained by <strong>Dizaraj Dey</strong>, a passionate WordPress developer dedicated to creating high-quality, user-friendly plugins.</p>
                        <h4>Contact Details:</h4>
                        <p>For support, inquiries, or custom development, please feel free to reach out. Your feedback is always welcome!</p>
                        <ul style="list-style: none; padding-left: 0;">
                            <li style="margin-bottom: 10px;"><strong>Email:</strong> <a href="mailto:dizaraj@gmail.com">dizaraj@gmail.com</a></li>
                            <li style="margin-bottom: 10px;"><strong>Messenger:</strong> <a href="https://m.me/dizaraj" target="_blank">m.me/dizaraj</a></li>
                            <li style="margin-bottom: 10px;"><strong>WhatsApp:</strong> <a href="https://wa.me/8801717035081" target="_blank">8801717035081</a></li>
                        </ul>
                    </div>
                </div>
             ...
             <div class="wcw-pro-settings-sidebar">
                <div class="wcw-pro-sidebar-box">
                    <h3><span class="dashicons dashicons-info-outline"></span> <?php esc_html_e( 'Essential Info', 'wcw-pro' ); ?></h3>
                    <div class="wcw-pro-essential-info">
                        <?php $info = wcw_pro_get_essential_info(); ?>
                        <ul>
                            <li><strong><?php esc_html_e('Domain:', 'wcw-pro'); ?></strong> <span><?php echo esc_html($info['domain']); ?></span></li>
                            <li><strong><?php esc_html_e('Server IP:', 'wcw-pro'); ?></strong> <span><?php echo esc_html($info['server_ip']); ?></span></li>
                            <li><strong><?php esc_html_e('Your Public IP:', 'wcw-pro'); ?></strong> <span><?php echo esc_html($info['your_public_ip']); ?></span></li>
                            <li><strong><?php esc_html_e('PHP Version:', 'wcw-pro'); ?></strong> <span><?php echo esc_html($info['php_version']); ?></span></li>
                            <li><strong><?php esc_html_e('WordPress Version:', 'wcw-pro'); ?></strong> <span><?php echo esc_html($info['wp_version']); ?></span></li>
                            <li><strong><?php esc_html_e('Memory Limit:', 'wcw-pro'); ?></strong> <span><?php echo esc_html($info['memory_limit']); ?></span></li>
                            <li><strong><?php esc_html_e('Memory Usage:', 'wcw-pro'); ?></strong> <span><?php echo esc_html($info['memory_usage']); ?></span></li>
                        </ul>
                    </div>
                </div>
            </div>
...
function wcw_pro_settings_page_html() {
    ...
    ?>
    <div class="wrap wcw-pro-settings-wrap">
        <h1><?php esc_html_e( 'WhatsApp Chat Settings', 'wcw-pro' ); ?></h1>
        <div class="wcw-pro-settings-content">
            <div class="wcw-pro-settings-main">
...
/**
 * Render the API Key field with masking.
 * @since 2.6.8
 */
function wcw_pro_api_key_field_callback() {
    $options = get_option('wcw_pro_settings');
...
function wcw_pro_handle_trial_request() {
    ...
    // Send email notification with visitor's IP info
    $visitor_data = wcw_pro_get_visitor_public_ip_and_location();
    $domain = home_url();
    ...
    $message = sprintf(
        'Success! Your 7-day trial key has been generated: %s. Please copy this key, paste it into the License Key field, then click "Save Settings".',
...
function wcw_pro_enqueue_assets() {
    wp_enqueue_style( 'wcw-pro-style', plugin_dir_url( __FILE__ ) . 'assets/css/style.css', array(), '2.8.0' );
    wp_enqueue_script( 'wcw-pro-script', plugin_dir_url( __FILE__ ) . 'assets/js/main.js', array( 'jquery' ), '2.8.0', true );
}
 ...
 * @since 2.3.0 (updated 2.8.0)
 */
function wcw_pro_enqueue_admin_scripts( $hook ) {
    $screen = get_current_screen();
    if ( ! $screen || strpos( $screen->id, 'wcw-pro' ) === false ) {
        return;
    }
    ...
    wp_enqueue_style( 'wcw-pro-admin-style', plugin_dir_url( __FILE__ ) . 'assets/css/admin.css', array(), '2.8.0' );
    wp_enqueue_script( 'wcw-pro-admin-script', plugin_dir_url( __FILE__ ) . 'assets/js/admin.js', array( 'jquery', 'wp-util', 'wp-color-picker', 'jquery-ui-accordion' ), '2.8.0', true );
 ...
 * This function should only be run when necessary, like on activation or a version change.
 * @since 2.7.0
 */
function wcw_pro_create_asset_files() {
    ...
    // Static CSS for frontend layout.
    ...
    // Admin CSS
    $admin_css_content = '/* Admin Settings Styles */.wcw-pro-settings-wrap{margin-top:20px}.wcw-pro-settings-content{display:flex;gap:20px;margin-top:0}.wcw-pro-settings-main{flex:2;background:#fff;padding:1px 20px 20px;border:1px solid #c3c4c7;border-radius:4px}.wcw-pro-settings-sidebar{flex:1;max-width:300px}.wcw-pro-sidebar-box{background:#fff;padding:20px;border:1px solid #c3c4c7;border-radius:4px;margin-bottom:20px}.wcw-pro-sidebar-box h3{margin-top:0;padding-bottom:10px;border-bottom:1px solid #ddd;display:flex;align-items:center;gap:8px}.wcw-pro-nav-tab-wrapper{margin-bottom:1px;border-bottom:1px solid #c3c4c7}.wcw-pro-nav-tab-wrapper .nav-tab{border:1px solid transparent;border-top:1px solid #c3c4c7;border-left:1px solid #c3c4c7;border-right:1px solid #c3c4c7;margin:0 5px -1px 0;background:#f0f0f1;border-top-left-radius:3px;border-top-right-radius:3px}.wcw-pro-nav-tab-wrapper .nav-tab-active{background:#fff;border-color:#c3c4c7;border-bottom-color:#fff}.wcw-pro-tab-content{display:none;padding-top:20px}.wcw-pro-tab-content.active{display:block}.wcw-pro-settings-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:20px}.wcw-pro-settings-card{background:#fff;border:1px solid #e0e0e0;border-radius:4px;box-shadow:0 1px 1px rgba(0,0,0,.04)}.wcw-pro-card-header{padding:15px 20px;border-bottom:1px solid #e0e0e0;background:#fafafa}.wcw-pro-card-header h3{margin:0;font-size:16px;display:flex;align-items:center;gap:10px}.wcw-pro-card-body{padding:20px}.wcw-pro-form-group{margin-bottom:15px}.wcw-pro-form-group:last-child{margin-bottom:0}.wcw-pro-form-group label{display:block;margin-bottom:5px;font-weight:600}.wcw-pro-radio-label{margin-right:20px;display:inline-flex;align-items:center;gap:5px}/* New Agent Form Styles */.wcw-pro-repeater-item{border:1px solid #e0e0e0;border-radius:4px;margin-bottom:15px;background:#fff;box-shadow:0 1px 1px rgba(0,0,0,.04);overflow:hidden}.wcw-pro-repeater-handle{display:flex;justify-content:space-between;align-items:center;padding:15px 20px;background:#fafafa;border-bottom:1px solid #e0e0e0;cursor:pointer;transition:background .2s}.wcw-pro-repeater-handle:hover{background:#f5f5f5}.wcw-pro-repeater-handle h3{margin:0;font-size:16px;display:flex;align-items:center;gap:10px;color:#2271b1}.wcw-pro-repeater-handle .wcw-pro-handle-arrow{transition:transform .2s;font-size:20px}.wcw-pro-repeater-item.open>.wcw-pro-repeater-handle .wcw-pro-handle-arrow{transform:rotate(180deg)}.wcw-pro-repeater-content{display:none;padding:20px}.wcw-pro-agent-grid{display:grid;grid-template-columns:150px 1fr;gap:30px;align-items:flex-start}.wcw-pro-agent-avatar-section{text-align:center}.wcw-pro-image-preview{width:100px;height:100px;border:2px solid #ddd;background:#f0f0f1;margin:0 auto 15px;border-radius:50%;overflow:hidden}.wcw-pro-image-preview img{width:100%;height:100%;object-fit:cover}.wcw-pro-agent-avatar-section .button{margin-bottom:5px}.wcw-pro-agent-avatar-section .wcw-pro-remove-image-button{color:#b32d2e}.wcw-pro-agent-details-section{display:flex;flex-direction:column;gap:15px}.wcw-pro-form-row{display:flex;gap:20px}.wcw-pro-form-group{flex:1;display:flex;flex-direction:column}.wcw-pro-form-group label{margin-bottom:5px;font-weight:600;color:#333}.wcw-pro-form-group input,.wcw-pro-form-group textarea{padding:8px 12px;border:1px solid #ddd;border-radius:4px;box-shadow:inset 0 1px 2px rgba(0,0,0,.07)}.wcw-pro-repeater-footer{margin-top:20px;text-align:right}.wcw-pro-repeater-footer .wcw-pro-remove-agent{color:#b32d2e;text-decoration:none;display:inline-flex;align-items:center;gap:5px}.wcw-pro-repeater-footer .wcw-pro-remove-agent:hover{color:#000}.wcw-pro-add-agent-wrapper .button-large{font-size:16px;height:46px;line-height:44px;padding:0 20px;display:inline-flex;align-items:center;gap:8px}/* Locked Pro Fields */.wcw-pro-field-wrapper{position:relative}.wcw-pro-field-wrapper.wcw-pro-locked{opacity:.6;pointer-events:none}.wcw-pro-badge{position:absolute;top:-10px;right:-10px;background:#ffba00;color:#fff;font-size:10px;font-weight:700;padding:2px 6px;border-radius:10px;text-transform:uppercase;z-index:1}#wcw-pro-preview-container{background:#f0f0f1;padding:20px;text-align:center;border:1px dashed #ccc;border-radius:4px}#wcw-pro-preview-bubble{width:60px;height:60px;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto;transition:all .2s}#wcw-pro-preview-bubble.has-text{width:auto;padding:0 20px 0 15px;border-radius:30px}#wcw-pro-preview-bubble.has-text #wcw-pro-preview-icon{margin-right:8px}#wcw-pro-preview-text{font-size:16px;font-weight:700;white-space:nowrap;display:none}#wcw-pro-preview-bubble.has-text #wcw-pro-preview-text{display:inline}#wcw-pro-preview-icon path{transition:fill .2s}/* Essential Info Box */.wcw-pro-essential-info ul{list-style:none;margin:0;padding:0}.wcw-pro-essential-info li{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #f0f0f1}.wcw-pro-essential-info li:last-child{border-bottom:none}.wcw-pro-essential-info li strong{font-weight:600}.wcw-pro-essential-info li span{color:#50575e;word-break:break-all}.wcw-pro-license-status p{padding:10px;border-radius:4px;margin:0}.status-valid{background-color:#e0f2e1;border-left:4px solid #4CAF50}.status-trial{background-color:#fff3e0;border-left:4px solid #ff9800}.status-expired,.status-invalid{background-color:#ffeb ee;border-left:4px solid #f44336}#wcw-pro-trial-message{padding:10px;border-radius:4px;border-left-width:4px;border-left-style:solid}#wcw-pro-trial-message.updated{background-color:#e0f2e1;border-color:#4CAF50}#wcw-pro-trial-message.error{background-color:#ffeb ee;border-color:#f44336}';
    file_put_contents($plugin_dir . 'assets/css/admin.css', $admin_css_content);
    
    // **BUG FIX** Corrected the minified JS string for the frontend popup.
