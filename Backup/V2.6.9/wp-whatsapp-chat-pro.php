...
/**
 * Get visitor's public IP and location for trial requests.
 * @return array
 * @since 2.6.9
 */
function wcw_pro_get_visitor_public_ip_and_location() {
    ...
    // Use a reliable external service to get the public IP of the visitor.
    // The server makes the request, but we forward the visitor's IP if available.
    $headers = [];
    ...
    return ['ip' => sanitize_text_field($ip), 'location' => ['city' => 'Unknown', 'country' => 'Unknown']];
}

/**
 * Get the server's public IP and cache it.
 * @return string
 * @since 2.6.9
 */
function wcw_pro_get_server_public_ip() {
    // Check for cached IP first.
    $cached_ip = get_transient('wcw_pro_server_public_ip');
    if (false !== $cached_ip) {
        return $cached_ip;
    }

    // If not cached, fetch it from a reliable service.
    ...
    if (filter_var($ip, FILTER_VALIDATE_IP)) {
        // Cache the valid IP for 6 hours to reduce API calls.
        set_transient('wcw_pro_server_public_ip', $ip, 6 * HOUR_IN_SECONDS);
        return $ip;
    }
...
function wcw_pro_get_essential_info() {
    ...
    return [
        ...
        'public_ip'     => wcw_pro_get_server_public_ip(),
                ...
                <div class="wcw-pro-sidebar-box">
                    ...
                    <div class="wcw-pro-essential-info">
                        ...
                        <ul>
                            ...
                            <li><strong><?php esc_html_e('Public IP:', 'wcw-pro'); ?></strong> <span><?php echo esc_html($info['public_ip']); ?></span></li>
...
function wcw_pro_enqueue_assets() {
    wp_enqueue_style( 'wcw-pro-style', plugin_dir_url( __FILE__ ) . 'assets/css/style.css', array(), '2.6.9' );
    wp_enqueue_script( 'wcw-pro-script', plugin_dir_url( __FILE__ ) . 'assets/js/main.js', array( 'jquery' ), '2.6.9', true );
}
...
function wcw_pro_enqueue_admin_scripts( $hook ) {
    ...
    wp_enqueue_style( 'wcw-pro-admin-style', plugin_dir_url( __FILE__ ) . 'assets/css/admin.css', array(), '2.6.9' );
    wp_enqueue_script( 'wcw-pro-admin-script', plugin_dir_url( __FILE__ ) . 'assets/js/admin.js', array( 'jquery', 'wp-util', 'wp-color-picker', 'jquery-ui-accordion' ), '2.6.9', true );
