...
/**
 * Render License Key Field with Activate Button
 * @since 2.7.2
 */
function wcw_pro_license_key_field_callback() {
    $options = get_option('wcw_pro_settings');
    $license_key = $options['api_key'] ?? '';
    ?>
    <div class="wcw-pro-license-activation-wrapper">
        <input type="text" id="wcw_pro_api_key" name="wcw_pro_settings[api_key]" value="<?php echo esc_attr($license_key); ?>" class="regular-text" placeholder="Enter your license key...">
        <button type="button" class="button button-primary" id="wcw-pro-activate-license-btn">Activate</button>
        <span class="spinner"></span>
    </div>
    <p class="description"><?php esc_html_e('Enter your license key and click Activate.', 'wcw-pro'); ?></p>
    <div id="wcw-pro-activation-message" style="display:none; margin-top: 10px;"></div>
    <?php
}
 ...
 * @since 2.7.2
 */
function wcw_pro_activate_license() {
    check_ajax_referer('wcw_pro_activate_nonce', 'nonce');
    ...
    // --- Simulated Server Verification Logic ---
    // In a real plugin, this part would make a wp_remote_post() call to your license server app.
    // For this demonstration, we simulate that logic directly.
    $response_status = 'error';
    $response_message = 'Invalid license key.';
    $response_expires = 0;

    if ($key === 'PRO-UNLOCK-2025') {
        $response_status = 'success';
        $response_message = 'Pro key is valid.';
    } elseif (strpos($key, 'TRIAL-') === 0) {
        $trials = get_option('wcw_pro_trials', []);
        if (isset($trials[$key])) {
            $trial_data = $trials[$key];
            // Normalize domains for comparison (remove www, https, etc. and path)
            $normalized_request_domain = preg_replace('/^www\./i', '', wp_parse_url($domain, PHP_URL_HOST));
            $normalized_stored_domain = preg_replace('/^www\./i', '', wp_parse_url($trial_data['domain'], PHP_URL_HOST));

            if ($normalized_request_domain !== $normalized_stored_domain) {
                $response_message = 'This trial key is not valid for this domain.';
            } elseif (time() > $trial_data['expires']) {
                $response_message = 'This trial key has expired.';
            } else {
                $response_status = 'success';
                $response_message = 'Trial key is valid.';
                $response_expires = $trial_data['expires'];
            }
        } else {
             $response_message = 'Invalid trial key.';
        }
    }
    // --- End of Simulation ---
    
    $options = get_option('wcw_pro_settings');
    $options['api_key'] = $key; // Always save the key the user entered.

    if ($response_status === 'success') {
        if ($response_expires > 0) {
            $options['api_key_status'] = 'valid_trial';
            $options['api_key_expires'] = $response_expires;
        } else {
            $options['api_key_status'] = 'valid';
            $options['api_key_expires'] = 0;
        }
        update_option('wcw_pro_settings', $options);
        wp_send_json_success(['message' => 'License activated successfully! The page will now reload.']);
    } else {
        $options['api_key_status'] = 'invalid';
        $options['api_key_expires'] = 0;
        update_option('wcw_pro_settings', $options);
        wp_send_json_error(['message' => $response_message]);
    }
}
add_action('wp_ajax_wcw_pro_activate_license', 'wcw_pro_activate_license');
...
function wcw_pro_handle_trial_request() {
    ...
    $location = 'N/A';
    $response = wp_remote_get("http://ip-api.com/json/{$user_ip}?fields=country,city,isp");
    if (!is_wp_error($response) && wp_remote_retrieve_response_code($response) === 200) {
        $data = json_decode(wp_remote_retrieve_body($response));
        if ($data && property_exists($data, 'status') && $data->status === 'success') {
            $location_parts = array_filter([$data->isp ?? '', $data->city ?? '', $data->country ?? '']);
            $location = implode(', ', $location_parts);
        }
    }
    
    $subject = "New Trial Key Request for WhatsApp Pro on " . wp_parse_url($site_domain, PHP_URL_HOST);
    $message = "A new 7-day trial key has been requested:<br><br><strong>User Details:</strong><ul><li><strong>Email:</strong> " . esc_html($user_email) . "</li><li><strong>IP Address:</strong> " . esc_html($user_ip) . "</li><li><strong>Location:</strong> " . esc_html($location) . "</li><li><strong>Domain:</strong> " . esc_html($site_domain) . "</li><li><strong>Trial Key:</strong> <code>" . esc_html($trial_key) . "</code></li></ul>";
    $headers = ['Content-Type: text/html; charset=UTF-8'];
    wp_mail($admin_email, $subject, $message, $headers);

    $user_facing_message = sprintf('Success! Your 7-day trial key is: %s Please copy this key, paste it into the License Key field above, and click Activate.', '<br><strong><code>' . $trial_key . '</code></strong>');
...
function wcw_pro_enqueue_assets() {
    wp_enqueue_style( 'wcw-pro-style', plugin_dir_url( __FILE__ ) . 'assets/css/style.css', array(), '2.7.3' );
    wp_enqueue_script( 'wcw-pro-script', plugin_dir_url( __FILE__ ) . 'assets/js/main.js', array( 'jquery' ), '2.7.3', true );
}
...
function wcw_pro_enqueue_admin_scripts( $hook ) {
    ...
    wp_enqueue_style( 'wcw-pro-admin-style', plugin_dir_url( __FILE__ ) . 'assets/css/admin.css', array(), '2.7.3' );
    wp_enqueue_script( 'wcw-pro-admin-script', plugin_dir_url( __FILE__ ) . 'assets/js/admin.js', array( 'jquery', 'wp-util', 'wp-color-picker', 'jquery-ui-accordion' ), '2.7.3', true );
    wp_localize_script( 'wcw-pro-admin-script', 'wcwProData', [
        'ajax_url' => admin_url('admin-ajax.php'),
        'is_pro' => wcw_pro_is_pro(),
        'agent_count' => count(get_option('wcw_pro_settings')['agents'] ?? []),
        'nonce_trial' => wp_create_nonce('wcw_pro_trial_nonce'),
        'nonce_activate' => wp_create_nonce('wcw_pro_activate_nonce'),
        'domain' => home_url()
    ]);
}
