...
function wcw_pro_add_admin_menu() {
    add_menu_page(
        ...
        esc_html__( 'WhatsApp Pro', 'wcw-pro' ),
        'manage_options',
        'wcw-pro-settings',
...
function wcw_pro_settings_init() {
    ...
    add_settings_section(
        'wcw_pro_main_section',
        esc_html__( 'Display Settings', 'wcw-pro' ),
        'wcw_pro_main_section_callback',
        'wcw-pro-settings'
    );

    add_settings_field(
        'wcw_pro_popup_title_field',
        esc_html__( 'Popup Title', 'wcw-pro' ),
        'wcw_pro_render_field',
        'wcw-pro-settings',
        'wcw_pro_main_section',
        ['type' => 'text', 'id' => 'popup_title', 'description' => 'The title displayed at the top of the chat popup.']
    );

    add_settings_field(
        'wcw_pro_bubble_color_field',
        esc_html__( 'Bubble Background Color', 'wcw-pro' ),
        'wcw_pro_render_field',
        'wcw-pro-settings',
        'wcw_pro_main_section',
        ['type' => 'color', 'id' => 'bubble_color', 'default' => '#25D366']
    );

     add_settings_field(
        'wcw_pro_icon_color_field',
        esc_html__( 'Bubble Icon Color', 'wcw-pro' ),
        'wcw_pro_render_field',
        'wcw-pro-settings',
        'wcw_pro_main_section',
        ['type' => 'color', 'id' => 'icon_color', 'default' => '#FFFFFF']
    );

    add_settings_field(
        'wcw_pro_agents_field',
        esc_html__( 'Manage Agents', 'wcw-pro' ),
        'wcw_pro_agents_field_callback',
        'wcw-pro-settings',
        'wcw_pro_main_section'
    );
}
...
function wcw_pro_sanitize_settings( $input ) {
    ...
    if( isset( $input['bubble_color'] ) ) {
        $new_input['bubble_color'] = sanitize_hex_color( $input['bubble_color'] );
    }
...
/**
 * Render a settings section description.
 * @since 2.0.0
 */
function wcw_pro_main_section_callback() {
    echo '<p>' . esc_html__( 'Customize the appearance and general settings of your WhatsApp chat widget.', 'wcw-pro' ) . '</p>';
}
                    ...
                    <div class="wcw-pro-repeater-item">
                        ...
                        <div class="wcw-pro-repeater-content">
                            <p>
                                ...
                                <input class="widefat" name="wcw_pro_settings[agents][<?php echo $index; ?>][name]" type="text" value="<?php echo esc_attr( $agent['name'] ); ?>">
                            </p>
                            <p>
                                <label><?php esc_html_e( 'Agent Title/Role:', 'wcw-pro' ); ?></label>
                                <input class="widefat" name="wcw_pro_settings[agents][<?php echo $index; ?>][title]" type="text" value="<?php echo esc_attr( $agent['title'] ); ?>">
                            </p>
                            <p>
                                <label><?php esc_html_e( 'Department:', 'wcw-pro' ); ?></label>
                                <input class="widefat" name="wcw_pro_settings[agents][<?php echo $index; ?>][department]" type="text" value="<?php echo esc_attr( $agent['department'] ); ?>">
                            </p>
                            <p>
                                <label><?php esc_html_e( 'WhatsApp Number (with country code):', 'wcw-pro' ); ?></label>
                                <input class="widefat" name="wcw_pro_settings[agents][<?php echo $index; ?>][phone]" type="text" value="<?php echo esc_attr( $agent['phone'] ); ?>">
                            </p>
                             <p>
                                <label><?php esc_html_e( 'Prefilled Message:', 'wcw-pro' ); ?></label>
                                <input class="widefat" name="wcw_pro_settings[agents][<?php echo $index; ?>][message]" type="text" value="<?php echo esc_attr( $agent['message'] ); ?>">
                            </p>
    ...
    <script type="text/template" id="tmpl-wcw-pro-repeater-template">
        <div class="wcw-pro-repeater-item">
            ...
            <div class="wcw-pro-repeater-content">
                <p>
                    <label><?php esc_html_e( 'Agent Name:', 'wcw-pro' ); ?></label>
                    <input class="widefat" name="wcw_pro_settings[agents][<%= index %>][name]" type="text" value="">
                </p>
                <p>
                    <label><?php esc_html_e( 'Agent Title/Role:', 'wcw-pro' ); ?></label>
                    <input class="widefat" name="wcw_pro_settings[agents][<%= index %>][title]" type="text" value="">
                </p>
                <p>
                    <label><?php esc_html_e( 'Department:', 'wcw-pro' ); ?></label>
                    <input class="widefat" name="wcw_pro_settings[agents][<%= index %>][department]" type="text" value="">
                </p>
                <p>
                    <label><?php esc_html_e( 'WhatsApp Number (with country code):', 'wcw-pro' ); ?></label>
                    <input class="widefat" name="wcw_pro_settings[agents][<%= index %>][phone]" type="text" value="">
                </p>
                <p>
                    <label><?php esc_html_e( 'Prefilled Message:', 'wcw-pro' ); ?></label>
                    <input class="widefat" name="wcw_pro_settings[agents][<%= index %>][message]" type="text" value="">
                </p>
                <a href="#" class="button wcw-pro-remove-agent"><?php esc_html_e( 'Remove Agent', 'wcw-pro' ); ?></a>
            </div>
        </div>
    ...
    <div class="wrap wcw-pro-settings-wrap">
        ...
        <div class="wcw-pro-settings-main">
            <form action="options.php" method="post">
                <?php
                settings_fields( 'wcw_pro_options_group' );
                do_settings_sections( 'wcw-pro-settings' );
...
// =============================================================================
// FRONTEND WIDGET & WOOCOMMERCE BUTTON
// =============================================================================
...
function wcw_pro_render_frontend_widget() {
    ...
    <div class="wcw-pro-container">
        <div class="wcw-pro-bubble" style="background-color: <?php echo esc_attr( $bubble_color ); ?>;">
             <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" width="30" height="30" fill="<?php echo esc_attr( $icon_color ); ?>">
                <path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.8 0-67.3-10.2-96.1-29.1l-6.7-4-71.6 18.7 19.3-68.6-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5c0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.8-16.2-54.3-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7.9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.9 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z"/>
            </svg>
        </div>
                    ...
                    <a href="<?php echo esc_url( $whatsapp_url ); ?>" target="_blank" rel="noopener noreferrer" class="wcw-pro-agent">
                        <div class="wcw-pro-agent-avatar">
                           <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" width="40" height="40">
                               <path fill="#cccccc" d="M224 256c70.7 0 128-57.3 128-128S294.7 0 224 0S96 57.3 96 128s57.3 128 128 128zm-45.7 48C79.8 304 0 383.8 0 482.3C0 498.7 13.3 512 29.7 512H418.3c16.4 0 29.7-13.3 29.7-29.7C448 383.8 368.2 304 269.7 304H178.3z"/>
                           </svg>
                        </div>
...
/**
 * Add WhatsApp button to WooCommerce product pages
 * @since 2.0.0
 */
function wcw_pro_add_wc_button() {
    ...
    if ( !empty($agents[0]) ) {
         ...
         $phone_number = preg_replace( '/\D/', '', $first_agent['phone'] );
         $product_title = get_the_title();
         $prefilled_message = sprintf(esc_html__('Hello, I am interested in the product: %s', 'wcw-pro'), $product_title);
         $whatsapp_url = 'https://wa.me/' . esc_attr( $phone_number ) . '?text=' . urlencode( $prefilled_message );

        $button_html = '<a href="' . esc_url( $whatsapp_url ) . '" target="_blank" rel="noopener noreferrer" class="button alt wcw-pro-wc-button">';
        $button_html .= '<svg aria-hidden="true" role="img" focusable="false" class="wcw-pro-wc-button-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" width="18" height="18"><path fill="currentColor" d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.8 0-67.3-10.2-96.1-29.1l-6.7-4-71.6 18.7 19.3-68.6-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5c0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.8-16.2-54.3-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7.9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.9 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z"></path></svg>';
        $button_html .= ' ' . esc_html__('Chat on WhatsApp', 'wcw-pro') . '</a>';

        echo $button_html;
    }
}
add_action( 'woocommerce_single_product_summary', 'wcw_pro_add_wc_button', 31 );
...
function wcw_pro_enqueue_assets() {
    wp_enqueue_style( 'wcw-pro-style', plugin_dir_url( __FILE__ ) . 'assets/css/style.css', array(), '2.0.0' );
    wp_enqueue_script( 'wcw-pro-script', plugin_dir_url( __FILE__ ) . 'assets/js/main.js', array( 'jquery' ), '2.0.0', true );
}
...
function wcw_pro_enqueue_admin_scripts( $hook ) {
    if ( 'toplevel_page_wcw-pro-settings' !== $hook ) {
        return;
    }
    wp_enqueue_style( 'wp-color-picker' );
    wp_enqueue_style( 'wcw-pro-admin-style', plugin_dir_url( __FILE__ ) . 'assets/css/admin.css', array(), '2.0.0' );
    wp_enqueue_script( 'wcw-pro-admin-script', plugin_dir_url( __FILE__ ) . 'assets/js/admin.js', array( 'jquery', 'wp-util', 'wp-color-picker', 'jquery-ui-accordion' ), '2.0.0', true );
}
...
function wcw_pro_activate() {
    ...
    if ( false === get_option( 'wcw_pro_settings' ) ) {
        // Check for old widget settings to migrate
        $old_widget_settings = get_option( 'widget_wcw_pro_widget' );
        if ( ! empty( $old_widget_settings ) && is_array( $old_widget_settings ) ) {
            $first_instance = reset($old_widget_settings);
            $defaults = [
                'popup_title' => $first_instance['title'] ?? esc_html__( 'Start a Conversation', 'wcw-pro' ),
                'bubble_color' => $first_instance['bubble_color'] ?? '#25D366',
                'icon_color' => $first_instance['icon_color'] ?? '#FFFFFF',
                'agents' => $first_instance['agents'] ?? []
            ];
        } else {
            // Setup fresh defaults
            $defaults = [
...
/**
 * The following functions are for creating the asset files on activation.
 * This is for demonstration. In a production plugin, these files would be included.
 */
function wcw_pro_create_asset_files() {
    ...
    if ( ! file_exists( $css_file ) ) {
        file_put_contents($css_file, '/* Main Frontend Styles */
.wcw-pro-container{position:fixed;bottom:25px;right:25px;z-index:1000}.wcw-pro-bubble{width:60px;height:60px;border-radius:50%;color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 4px 12px rgba(0,0,0,.2);transition:transform .2s ease-in-out}.wcw-pro-bubble:hover{transform:scale(1.1)}.wcw-pro-popup{position:absolute;bottom:80px;right:0;width:320px;background:#fff;border-radius:10px;box-shadow:0 5px 15px rgba(0,0,0,.2);opacity:0;visibility:hidden;transform:scale(.95) translateY(10px);transform-origin:bottom right;transition:all .2s ease-in-out}.wcw-pro-popup.open{opacity:1;visibility:visible;transform:scale(1) translateY(0)}.wcw-pro-header{padding:15px;border-top-left-radius:10px;border-top-right-radius:10px;display:flex;justify-content:space-between;align-items:center}.wcw-pro-title{font-weight:700}.wcw-pro-close{font-size:24px;cursor:pointer;line-height:1}.wcw-pro-body{padding:10px;max-height:400px;overflow-y:auto}.wcw-pro-agent{display:flex;align-items:center;padding:10px;text-decoration:none;color:#333;border-radius:8px;transition:background-color .2s}.wcw-pro-agent:hover{background-color:#f5f5f5}.wcw-pro-agent-avatar{margin-right:15px}.wcw-pro-agent-details{display:flex;flex-direction:column}.wcw-pro-agent-name{font-weight:700}.wcw-pro-agent-title{font-size:.9em;color:#777}.wcw-pro-wc-button{margin-top:10px!important;color:#fff!important;width:100%;text-align:center;display:flex;align-items:center;justify-content:center}.wcw-pro-wc-button:hover{color:#fff!important}.wcw-pro-wc-button-svg{margin-right:8px}');
    }

    $admin_css_file = $plugin_dir . 'assets/css/admin.css';
    if ( ! file_exists( $admin_css_file ) ) {
        file_put_contents($admin_css_file, '/* Admin Settings Styles */
.wcw-pro-settings-wrap{display:flex;gap:20px}.wcw-pro-settings-main{flex:2}.wcw-pro-settings-sidebar{flex:1;max-width:300px}.wcw-pro-repeater-item{padding:15px;border:1px solid #ddd;margin-bottom:10px;border-radius:4px;background:#fff}.wcw-pro-repeater-handle{margin:-15px -15px 10px;padding:10px 15px;background:#f9f9f9;border-bottom:1px solid #ddd;cursor:move;border-top-left-radius:4px;border-top-right-radius:4px}.wcw-pro-repeater-content{display:none}#wcw-pro-preview-container{background:#f0f0f1;padding:20px;text-align:center;border:1px dashed #ccc;border-radius:4px}#wcw-pro-preview-bubble{width:60px;height:60px;border-radius:50%;color:#fff;display:flex;align-items:center;justify-content:center;margin:0 auto;transition:background-color .2s}#wcw-pro-preview-icon{transition:fill .2s}');
    }

    $js_file = $plugin_dir . 'assets/js/main.js';
    if ( ! file_exists( $js_file ) ) {
        file_put_contents($js_file, "jQuery(document).ready(function(o){\"use strict\";o(\".wcw-pro-bubble\").on(\"click\",function(c){c.stopPropagation(),o(\".wcw-pro-popup\").toggleClass(\"open\")}),o(\".wcw-pro-close\").on(\"click\",function(){o(\".wcw-pro-popup\").removeClass(\"open\")}),o(document).on(\"click\",function(c){o(c.target).closest(\".wcw-pro-container\").length||o(\".wcw-pro-popup\").hasClass(\"open\")&&o(\".wcw-pro-popup\").removeClass(\"open\")})});");
    }

    $admin_js_file = $plugin_dir . 'assets/js/admin.js';
    if ( ! file_exists( $admin_js_file ) ) {
        file_put_contents($admin_js_file, "jQuery(function(e){\"use strict\";function t(t){e(\".wcw-pro-color-picker\",t).wpColorPicker({change:function(t,c){var r=e(t.target).attr(\"id\");\"bubble_color\"===r?e(\"#wcw-pro-preview-bubble\").css(\"background-color\",c.color.toString()):\"icon_color\"===r&&e(\"#wcw-pro-preview-icon\").attr(\"fill\",c.color.toString())}}),e(\"#wcw-pro-repeater\",t).accordion({header:\".wcw-pro-repeater-handle\",collapsible:!0,active:!1,heightStyle:\"content\"})}var c=e(\"#wcw-pro-repeater\"),r=e(\"#wcw-pro-add-agent\"),a=wp.template(\"wcw-pro-repeater-template\");e(\"#bubble_color\").trigger(\"change\"),e(\"#icon_color\").trigger(\"change\"),r.on(\"click\",function(t){t.preventDefault();var r={index:c.children(\".wcw-pro-repeater-item\").length};c.append(a(r)),c.accordion(\"refresh\")}),c.on(\"click\",\".wcw-pro-remove-agent\",function(t){t.preventDefault(),e(this).closest(\".wcw-pro-repeater-item\").remove(),c.accordion(\"refresh\"),c.find(\".wcw-pro-repeater-item\").each(function(t){var c=e(this);c.find(\"input, select, textarea\").each(function(){var e=c.find(this);e.attr(\"name\",function(e,c){if(c)return c.replace(/\[\\d+\]/,\"[\"+t+\"]\")})})})}),t(e(\".wcw-pro-settings-wrap\"))});");
    }
}
