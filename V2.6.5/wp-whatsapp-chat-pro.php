...
function wcw_pro_settings_init() {
    ...
    // General Section
    add_settings_section('wcw_pro_general_section', 'General Settings', '__return_false', 'wcw_pro_settings_general');
    ...
    // Display Section
    add_settings_section('wcw_pro_display_section', null, '__return_false', 'wcw_pro_settings_display');
...
function wcw_pro_sanitize_settings( $input ) {
    ...
    if (isset($input['api_key'])) {
        ...
        // If the submitted key contains asterisks, it's the masked version. Preserve the original key.
        if (strpos($submitted_key, '*') !== false) {
            $api_key = $options['api_key'] ?? '';
        } else {
            // A new key has been entered, so we use it.
            ...
            <div class="wcw-pro-settings-main">
                ...
                <form action="options.php" method="post">
                    ...
                    <div id="display" class="wcw-pro-tab-content">
                        <?php wcw_pro_display_settings_fields_callback(); ?>
                    </div>
...
function wcw_pro_enqueue_assets() {
    wp_enqueue_style( 'wcw-pro-style', plugin_dir_url( __FILE__ ) . 'assets/css/style.css', array(), '2.6.5' );
    wp_enqueue_script( 'wcw-pro-script', plugin_dir_url( __FILE__ ) . 'assets/js/main.js', array( 'jquery' ), '2.6.5', true );
}
...
function wcw_pro_enqueue_admin_scripts( $hook ) {
    ...
    wp_enqueue_style( 'wcw-pro-admin-style', plugin_dir_url( __FILE__ ) . 'assets/css/admin.css', array(), '2.6.5' );
    wp_enqueue_script( 'wcw-pro-admin-script', plugin_dir_url( __FILE__ ) . 'assets/js/admin.js', array( 'jquery', 'wp-util', 'wp-color-picker', 'jquery-ui-accordion' ), '2.6.5', true );
...
.wcw-pro-radio-label { margin-right: 20px; }
