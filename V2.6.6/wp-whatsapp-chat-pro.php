 ...
 * @since 2.6.4
 */
function wcw_pro_get_user_ip_and_location() {
    $ip = '';
    ...
    try {
        ...
        if ( is_array( $response ) && ! is_wp_error( $response ) ) {
            ...
            if ( $data && $data['status'] === 'success' ) {
                $location['city'] = sanitize_text_field($data['city']);
                $location['country'] = sanitize_text_field($data['country']);
            }
        }
    } catch (Exception $e) {
        // Do nothing, return default location.
    }

    return ['ip' => sanitize_text_field($ip), 'location' => $location];
}
...
function wcw_pro_sanitize_settings( $input ) {
    ...
    // License Key Handling with Masking Check
...
function wcw_pro_api_key_field_callback() {
    ...
    if ( ! empty( $value ) ) {
        if ( strpos( $value, 'PRO-' ) === 0 ) {
            $display_value = 'PRO-' . str_repeat( '*', strlen( $value ) - 4 );
        } elseif ( strpos( $value, 'TRIAL-' ) === 0 ) {
            $display_value = 'TRIAL-' . str_repeat( '*', strlen( $value ) - 6 );
        }
    }
 ...
 * @since 2.6.4 (Email Notification Update)
 */
function wcw_pro_handle_trial_request() {
    ...
    $user_data = wcw_pro_get_user_ip_and_location();
    $domain = home_url();
    ...
    $body .= "<li><strong>IP Address:</strong> " . esc_html($user_data['ip']) . "</li>";
    $body .= "<li><strong>Location:</strong> " . esc_html($user_data['location']['city']) . ", " . esc_html($user_data['location']['country']) . "</li>";
...
function wcw_pro_enqueue_assets() {
    wp_enqueue_style( 'wcw-pro-style', plugin_dir_url( __FILE__ ) . 'assets/css/style.css', array(), '2.6.6' );
    wp_enqueue_script( 'wcw-pro-script', plugin_dir_url( __FILE__ ) . 'assets/js/main.js', array( 'jquery' ), '2.6.6', true );
}
...
function wcw_pro_enqueue_admin_scripts( $hook ) {
    ...
    wp_enqueue_style( 'wcw-pro-admin-style', plugin_dir_url( __FILE__ ) . 'assets/css/admin.css', array(), '2.6.6' );
    wp_enqueue_script( 'wcw-pro-admin-script', plugin_dir_url( __FILE__ ) . 'assets/js/admin.js', array( 'jquery', 'wp-util', 'wp-color-picker', 'jquery-ui-accordion' ), '2.6.6', true );
...
function wcw_pro_create_asset_files() {
    ...
    // Static CSS for frontend layout. Dynamic styles are now handled by wcw_pro_output_dynamic_styles().
...
.wcw-pro-settings-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; }
.wcw-pro-settings-card { background: #fff; border: 1px solid #e0e0e0; border-radius: 4px; box-shadow: 0 1px 1px rgba(0,0,0,.04); }
.wcw-pro-card-header { padding: 15px 20px; border-bottom: 1px solid #e0e0e0; background: #fafafa; }
.wcw-pro-card-header h3 { margin: 0; font-size: 16px; display: flex; align-items: center; gap: 10px; }
.wcw-pro-card-body { padding: 20px; }
.wcw-pro-form-group { margin-bottom: 15px; }
.wcw-pro-form-group:last-child { margin-bottom: 0; }
.wcw-pro-form-group label { display: block; margin-bottom: 5px; font-weight: 600; }
.wcw-pro-radio-label { margin-right: 20px; display: inline-flex; align-items: center; gap: 5px; }
...
.wcw-pro-form-group input, .wcw-pro-form-group textarea {padding: 8px 12px; border: 1px solid #ddd; border-radius: 4px; box-shadow: inset 0 1px 2px rgba(0,0,0,.07); }
...
#wcw-pro-trial-message.error{background-color:#ffeb ee;border-color:#f44336}';
    ...
    // Admin JS - Updated for new Accordion and Display section
